...
export default function Chat() {
  ...
  // Reset the session: create a new session id and clear messages/input
  function resetSession() {
    const newId = `s_${Date.now()}_${Math.floor(Math.random() * 100000)}`;
    sessionStorage.setItem('user_session_id', newId);
    setMessages([
      { id: Date.now(), from: 'bot', text: "Hi — I'm Snackii. How can I help you today?" },
    ]);
    setInput('');
    console.log('Session reset, new id:', newId);
  }
      ...
      <div className="chat-container">
        <div className="chat-header">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <button className="reset-button" onClick={() => setShowConfirm(true)} aria-label="Reset session" title="Reset session?">
                <svg fill="#000000" height="24" width="24" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" 
                  ...
                  <g>
                    <path d="M42,12H20.414l7.293-7.293c0.391-0.391,0.391-1.023,0-1.414s-1.023-0.391-1.414,0l-8.999,8.999
                    ...
                    <path d="M35.707,37.293c-0.391-0.391-1.023-0.391-1.414,0s-0.391,1.023,0,1.414L41.586,46H18C9.178,46,2,38.822,2,30
        ...
        {showConfirm && (
          <div className="modal-overlay" role="dialog" aria-modal="true" aria-label="Reset chat confirmation">
            <div className="modal">
              <p style={{margin:0, marginBottom:12}}>Reset session and clear the chat?</p>
              <div className="modal-actions">
                <button className="confirm-button" onClick={() => { resetSession(); setShowConfirm(false); }}>Yes</button>
